...
VulkanContext::~VulkanContext() {
    ...
    // TODO: Create destructor for swapchain that handles this automatically!
    for (auto &imageResource: swapchain.images)
        vkDestroyImageView(device, imageResource.getImageView(), nullptr);
...
void VulkanContext::createSwapchain(VkExtent2D windowExtent) {
    ...
    vkb::Swapchain vkbSwapchain = builder.set_desired_format(VkSurfaceFormatKHR{
                .format = imageFormat, .colorSpace = VK_COLOR_SPACE_SRGB_NONLINEAR_KHR
            })
    ...
    const auto extent = vkbSwapchain.extent;
    const auto images = vkbSwapchain.get_images().value();
    const auto imageViews = vkbSwapchain.get_image_views().value();
    if (images.size() != imageViews.size()) {
        std::cout << "Failed to properly allocate swapchain images - number of views doesn't match number of images" <<
                std::endl;
        abort();
    }

    for (uint32_t i = 0; i < images.size(); i++) {
        auto imageResource = ImageResource{
            images[i], imageViews[i], extent, VK_IMAGE_LAYOUT_UNDEFINED, imageFormat
        };
